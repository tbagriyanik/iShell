...
const NewAppModal: React.FC<NewAppModalProps> = ({ isOpen, onClose, onSubmit, isGenerating, generationError, texts, themeColor }) => {
  ...
  const modalContentBgClass = 'bg-stone-800 dark:bg-neutral-900';
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-[1000] p-2 sm:p-4" 
        // onClick={onClose} // Prevent closing on overlay click if form is focused or during generation
        onClick={() => { if(!isGenerating) onClose();}}
        >
      <div 
        className={`flex flex-col rounded-lg shadow-2xl w-full max-w-md sm:max-w-lg relative border border-stone-700 dark:border-neutral-700 overflow-hidden ${modalContentBgClass}`}
      ...
      >
        ...
        <div className={`h-8 sm:h-9 ${modalTitleBarBgClass} text-white flex items-center justify-between px-2 sm:px-3 select-none shrink-0`}>
            <div className="flex items-center space-x-1.5">
                {/* Generic icon for new app modal, or could be PlusIcon */}
        ...
        {/* Content */}
        <div className="p-3 sm:p-5 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-600 text-white max-h-[calc(90vh-3rem)]">
            <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-4">
            <div>
                <label htmlFor="appName" className="block text-xs sm:text-sm font-medium text-gray-300 mb-1">{texts.appNameLabel}</label>
                <input
                type="text"
                id="appName"
                name="appName"
                ...
                required
                disabled={isGenerating}
                className={`w-full p-2 sm:p-2.5 rounded-md bg-stone-700 dark:bg-neutral-800 border border-stone-600 dark:border-neutral-700 focus:ring-1 ${focusRingClass} focus:border-${themeColor}-500 outline-none text-sm sm:text-base disabled:opacity-70`}
                placeholder={texts.appNamePlaceholder}
                />
            </div>
            <div>
                <label htmlFor="aiPrompt" className="block text-xs sm:text-sm font-medium text-gray-300 mb-1">{texts.aiPromptLabel}</label>
                <textarea
                id="aiPrompt"
                name="aiPrompt"
                ...
                disabled={isGenerating}
                rows={3} 
                className={`w-full p-2 sm:p-2.5 rounded-md bg-stone-700 dark:bg-neutral-800 border border-stone-600 dark:border-neutral-700 focus:ring-1 ${focusRingClass} focus:border-${themeColor}-500 outline-none resize-none scrollbar-thin scrollbar-thumb-stone-500 text-sm sm:text-base disabled:opacity-70`}
                placeholder={texts.aiPromptPlaceholder}
                />
            </div>

            <div>
                <label className="block text-xs sm:text-sm font-medium text-gray-300 mb-1.5">{texts.selectIcon}</label>
                <div className={`grid grid-cols-4 sm:grid-cols-5 gap-2 p-2 rounded-md ${isGenerating ? 'opacity-60' : 'bg-stone-700/50 dark:bg-neutral-800/50'}`}>
                {AVAILABLE_APP_ICONS.map((iconOption) => {
                    ...
                    >
                        ...
                        <span className="text-xs mt-1 truncate w-full text-center text-gray-300">{iconOption.name}</span>
                    </button>
                    );
                })}
                </div>
            ...
            {generationError && !isGenerating && (
                <p className="text-red-400 text-xs sm:text-sm bg-red-900/30 p-1.5 sm:p-2 rounded-md">{generationError}</p>
            )}
            {isGenerating && (
                 <div className="text-center py-2 text-sm text-yellow-400 flex items-center justify-center space-x-2">
